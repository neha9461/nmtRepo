...
@Slf4j
@Configuration
public class NoteTestDataRepository {
    private static HashMap<String, Note> articles;

    public NoteTestDataRepository() {
    }

    public synchronized HashMap<String, Note> getArticles() {

        if (articles == null) {
            articles = getTestDataMap(Note.class, "NoteTestData.json");
        }
        return articles;
    }
